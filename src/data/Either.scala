...
case class Left[+E](value: E) extends Either[E, Nothing] // convention: this is the failure case
...
object Either {
  ...
  def traverse[E,A,B](as: List[A])(f: A => Either[E,B]) : Either[E,List[B]] = {
    val fa = as map f
    // find a failure
    val e = Option.find(fa)(x => x match { case Left(e) => true case Right(_) => false })
    e match {
      case some(error) => error.map(b => List(b))
      // no failure found? then unwrap all the Bs
      case none => Right(fa flatMap (y => y match { case Left(_) => Nil case Right(v) => List(v) } ) )
    }
  } 
